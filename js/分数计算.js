...
// function get_artifact_score(point_mark, max_mark, artifact, element, pos_idx) {
...
//         calc_main = 0.0
//     } else {
//         calc_main = point_mark.get(main_name, 0) * artifact['主属性']['属性值'] * 46.6 / 6 / 100 / 4
//     }
//     // 副词条得分
//     var calc_subs = []
//     for (var i = 0; i < artifact.length; i++) {
//         var temp = artifact['词条'][i]
//         calc_subs.push([temp['属性名'], temp['属性值'], point_mark.get(temp['属性名'], 0) * temp['属性值'] * 46.6 / 6 / 100])
//     }
//     // [词条名, 词条数值, 词条得分]
//     for (var i = 0; i < calc_subs.length; i++) {
        
//     }
//     for sub in calc_subs:
//         if '攻击力' in sub && sub[2] > 15:
//             sub[2] = sub[1] * 0.398 * 0.5 * 0.75
//         if '防御力' in sub && sub[2] > 15:
//             sub[2] = sub[1] * 0.335 * 0.66 * 0.75
//         if '生命值' in sub && sub[2] > 15:
...
//             if k in sub_affixs && k != main_affix && affix_weight.get(k)
//         }
//         // 副词条中评分权重最高的词条得分大幅提升
...
//                     && artifacts[3]['主属性']['属性名'] == '百分比生命值' \
//                     && artifacts[4]['主属性']['属性名'] == '百分比生命值' \
//                     && data['属性']['暴击率'] * 2 + data['属性']['暴击伤害'] < 1 \
//                     && data['属性']['基础生命'] + data['属性']['基础生命'] > 40000:
//                 role_name = '钟离-血牛'
//         elif role_name == '芭芭拉':
//             if artifacts[3]['主属性']['属性名'] == '水元素伤害加成' && \
//                     data['属性']['暴击率'] * 2 + data['属性']['暴击伤害'] >= 1.8:
//                 role_name = '芭芭拉-暴力'
//         elif role_name == '甘雨':
//             suit = get_artifact_suit(artifacts)
//             if '冰' in suit[0][0] && '冰' in suit[1][0]:
//                 role_name = '甘雨-永冻'
//         elif role_name == '刻晴':
//             if data['属性']['元素精通'] > 80:
//                 role_name = '刻晴-精通'
...
//                 role_name = '温迪-充能'
//         elif role_name == '宵宫':
//             if data['属性']['元素精通'] < 50 && data['属性']['暴击率'] * 2 + data['属性']['暴击伤害'] > 3.2:
//                 role_name = '宵宫-纯火'
//             if data['属性']['元素精通'] > 200 && artifacts[2]['主属性']['属性名'] == '元素精通':
//                 role_name = '宵宫-精通'
//         elif role_name == '行秋':
//             if data['属性']['元素精通'] > 120:
//                 role_name = '行秋-蒸发'
...
//                     && artifacts[3]['主属性']['属性名'] == '岩元素伤害加成' \
//                     && artifacts[4]['主属性']['属性名'] in ['暴击率', '暴击伤害', '百分比防御力', '百分比攻击力']:
//                 role_name = '云堇-输出'
//         elif role_name == '雷电将军':
//             if data['属性']['元素精通'] > 500:
//                 role_name = '雷电将军-精通'
//             elif data['武器']['名称'] == '薙草之稻光' && data["武器"]["精炼等级"] >= 3:
//                 role_name = '雷电将军-高精'
//         elif role_name == '胡桃':
//             if data['属性']['暴击率'] < 0.15 && data['属性']['暴击伤害'] > 2.8:
//                 role_name = '胡桃-核爆'
//         elif role_name == '夜兰':
//             if data['属性']['元素精通'] > 120:
//                 role_name = '夜兰-精通'
//             if data['武器']['名称'] == '若水':
//                 role_name = '夜兰-若水'
//             if data['属性']['元素精通'] > 120 && data['武器']['名称'] == '若水':
//                 role_name = '夜兰-精通若水'
//         elif role_name == '神里绫华':
//             if data['属性']['元素精通'] > 120:
//                 role_name = '神里绫华-精通'
...
//                 role_name = '可莉-纯火'
//         elif role_name == '优菈':
//             if data['属性']['暴击率'] < 0.15 && data['属性']['暴击伤害'] > 2:
//                 role_name = '优菈-核爆'
...
//     :return: 是否有效
//     """
//     if ('攻击力' in effective or '百分比攻击力' in effective) && '攻击力' in prop_name:
//         return True
//     if ('生命值' in effective or '百分比生命值' in effective) && '生命值' in prop_name:
//         return True
//     if ('防御力' in effective or '百分比防御力' in effective) && '防御力' in prop_name:
//         return True
...
//     for s in suit:
//         if s not in suit2 && 1 < suit.count(s) < 4:
//             suit2.append(s)
//         if suit.count(s) >= 4:
//             for r in artifacts:
//                 if r['所属套装'] == s:
